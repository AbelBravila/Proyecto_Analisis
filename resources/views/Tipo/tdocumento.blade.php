<x-admin-layout>
    ...
    <b>
        <h1 class="text-lg text-center dark:text-black">TIPO DE DOCUMENTOS</h1>
    </b>

    <form method="GET" action="{{ route('Documento') }}" class="mb-4">
    ...
    <button data-modal-target="crud-modal" data-modal-toggle="crud-modal" class="block text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button">
      ...
      <div id="crud-modal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
        ...
        <div class="container">
            ...
            <form class="p-4 md:p-5" action="{{ route('Documento') }}" method="POST">
            @csrf
            <div class="grid gap-4 mb-4 grid-cols-2">
                        <div class="col-span-2">
                            <label for="nombre_documento" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nombre del Documento</label>
                            <input type="text" id="nombre_documento" name="nombre_documento" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required>
    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-white">
        ...
        <tbody>
            @foreach ($documentos as $documento)
                <tr>
                    <td scope="col" class="px-6 py-3 dark:text-black">{{ $documento->id_tipo_documento}}</td>
                    <td scope="col" class="px-6 py-3 dark:text-black">{{ $documento->nombre_documento}}</td>
                    <td scope="col" class="px-6 py-3 dark:text-black">{{ $documento->estado }}</td>
                    ...
                    <a class="fa fa-pencil fa-lg font-medium text-blue-600 dark:text-blue-500 hover:underline"
                                data-modal-target="editar-modal-documento-{{ $documento->id_tipo_documento }}"
                                data-modal-toggle="editar-modal-documento-{{ $documento->id_tipo_documento }}"
                                class="text-blue-600">
                    </a>
                    <a class="fa fa-trash fa-lg font-medium text-blue-600 dark:text-blue-500 hover:underline"
                                href="{{ route('Documento.cambiar_estado', ['id' => $documento->id_tipo_documento]) }}"
                                onclick="return confirm('¿Estás seguro de que deseas eliminar este tipo de documento?')">
                    </a>
                    </td>
                </tr>
                <div id="editar-modal-documento-{{ $documento->id_tipo_documento }}" tabindex="-1"
                        aria-hidden="true"
                        ...
                        <div class="relative p-4 w-full max-w-md max-h-full">
                            ...
                            <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                                ...
                                <!-- Modal body -->
                                <form
                                    action="{{ route('Documento.actualizar_documento', $documento->id_tipo_documento) }}"
                                    method="POST">
                                    @csrf
                                    @method('PUT')
                                    <div class="grid gap-4 mb-4 grid-cols-2">
